...
const FinanceOverview = () => {
  ...
  const recentPayments = [
    { id: 1, student: 'Sarah Johnson', class: 'Form 3A', amount: 45000, date: '2024-05-01', status: 'Paid', type: 'Tuition' },
    { id: 2, student: 'Michael Obi', class: 'Form 2B', amount: 35000, date: '2024-04-28', status: 'Partial', type: 'Tuition' },
    { id: 3, student: 'Fatima Ahmed', class: 'Form 1C', amount: 12000, date: '2024-04-27', status: 'Paid', type: 'Books' },
    { id: 4, student: 'David Maina', class: 'Form 4A', amount: 50000, date: '2024-04-25', status: 'Paid', type: 'Tuition' },
    { id: 5, student: 'Grace Owuor', class: 'Form 3B', amount: 28000, date: '2024-04-22', status: 'Overdue', type: 'Tuition' },
  ];

  const feeStructure = [
    { level: 'Form 1', tuition: 35000, boarding: 45000, activity: 10000, books: 15000, total: 105000 },
    { level: 'Form 2', tuition: 38000, boarding: 45000, activity: 10000, books: 12000, total: 105000 },
    { level: 'Form 3', tuition: 40000, boarding: 45000, activity: 15000, books: 18000, total: 118000 },
    { level: 'Form 4', tuition: 45000, boarding: 45000, activity: 20000, books: 20000, total: 130000 },
  ];
  ...
  const handlePaymentSubmit = () => {
    // Validate form
    if (!paymentForm.studentId || !paymentForm.paymentType || !paymentForm.amount || !paymentForm.paymentDate || !paymentForm.status) {
      toast({
              ...
              <Card className="shadow-sm hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-gray-500 text-sm font-medium">Total Revenue (2024)</h3>
              ...
              <Card className="shadow-sm hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold">Revenue Trend (2024)</h3>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={() => setShowAddPaymentModal(true)} className="bg-indigo-600 hover:bg-indigo-700 flex items-center gap-1">
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {recentPayments.map((payment) => (
                        <tr key={payment.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => openPaymentDetail(payment)}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">NGN {payment.amount.toLocaleString()}</td>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 flex items-center gap-1">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Activity Fees</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Books & Materials</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {feeStructure.map((fee, index) => (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">NGN {fee.tuition.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">NGN {fee.boarding.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">NGN {fee.activity.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">NGN {fee.books.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">NGN {fee.total.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-6 bg-blue-50 p-4 rounded-md border border-blue-200">
                <h4 className="text-blue-700 text-sm font-semibold mb-2">Payment Schedule</h4>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <Card className="shadow-sm hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-gray-500 text-sm font-medium">Largest Category</h3>
                    <FileText className="h-5 w-5 text-blue-500" />
                  </div>
                  <div className="flex items-baseline space-x-2">
                    <span className="text-2xl font-bold">Staff Salaries</span>
                    <span className="text-blue-500 text-xs font-semibold">45%</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="shadow-sm hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-gray-500 text-sm font-medium">Budget Status</h3>
                    <Check className="h-5 w-5 text-green-500" />
                  </div>
                  <div className="flex items-baseline space-x-2">
                    <span className="text-2xl font-bold">On Track</span>
                    <span className="text-green-500 text-xs font-semibold">+2% under</span>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="shadow-sm mb-6">
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold">Monthly Expenses (2024)</h3>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="flex items-center gap-1">
                      <Download className="h-4 w-4" />
                      Export
                    </Button>
                    <Button onClick={() => setShowExpenseModal(true)} size="sm" className="bg-indigo-600 hover:bg-indigo-700 flex items-center gap-1">
                      <Plus className="h-4 w-4" />
                      Add Expense
                    </Button>
                  </div>
                ...
                <div className="h-64 mb-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={monthlyRevenue}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="amount" name="Expense (NGN)" fill="#a855f7" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                <div className="bg-purple-50 p-4 rounded-md border border-purple-200 mb-6">
                  <h4 className="text-purple-700 text-sm font-semibold mb-2">Expense Categories</h4>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      <div className="flex items-center">
                        <input type="radio" id="pdf" name="format" className="mr-2" />
        ...
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex space-x-8" aria-label="Tabs">
            ...
            <button
              ...
              className={`${activeTab === 'fee-structure' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}
            >
              Fee Structure
            </button>
            <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowAddPaymentModal(false)} className="text-gray-500 hover:text-gray-700">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  {recentPayments.map((payment) => (
                    <option key={payment.id} value={payment.id}>{payment.student} - {payment.class}</option>
                  ))}
                </select>
              </div>
              <div>
