...
const StudentsManagement = () => {
  ...
  const filteredStudents = students.filter((student) => {
    ...
    const matchesPerformance = selectedPerformance === 'all' || (
      student.performance.averageGrade >= 80 ? 'high' :
      student.performance.averageGrade >= 60 ? 'medium' :
      'low'
    ) === selectedPerformance;
  ...
  const averageAttendance = Math.round(
    students.reduce((acc, s) => acc + s.performance.attendance, 0) / students.length
  );
  const averageGrade = Math.round(
    students.reduce((acc, s) => acc + s.performance.averageGrade, 0) / students.length
  );
  ...
  const handleQuickFilter = (filter: string) => {
    ...
    switch (filter) {
      case 'excellent':
        ...
        break;
      case 'attendance':
        setStudents([...students].sort((a, b) => b.performance.attendance - a.performance.attendance));
        break;
      case 'grades':
        setStudents([...students].sort((a, b) => b.performance.averageGrade - a.performance.averageGrade));
        break;
    }
  };
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div className="flex items-center text-sm">
                        ...
                        <span className="font-medium">{student.performance.attendance}%</span>
                      </div>
                      <div className="flex items-center text-sm">
                        <span className="w-24">Avg. Grade:</span>
                        <span className="font-medium">{student.performance.averageGrade}%</span>
                      </div>
                      <div className="flex items-center text-sm">
                        <span className="w-24">Subjects:</span>
                        <span className="font-medium">{student.performance.subjects.length}</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    {student.nextClass && (
                      <div className="text-sm">
                        <div className="font-medium">{student.nextClass.subject}</div>
                        <div className="text-gray-500">{student.nextClass.teacher}</div>
                        <div className="flex items-center text-gray-400">
                          <Clock className="h-3 w-3 mr-1" />
                          {student.nextClass.time}
                        </div>
                        <div className="text-gray-400">{student.nextClass.room}</div>
                      </div>
                    )}
                  </TableCell>
                  <TableCell>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl p-6">
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                {selectedStudentForPerformance.performance.subjects.map((subject, index) => (
                  <Card key={index}>
