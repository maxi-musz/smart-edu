...
const DashboardOverview: React.FC = () => {
  ...
  const todaysClasses = mockSchedule.filter(item => 
    ['Monday', 'Today', todayName].includes(item.day)
  ).slice(0, 3);
  
  const tomorrowsClasses = mockSchedule.filter(item => 
    ['Tuesday', 'Tomorrow', tomorrowName].includes(item.day)
  ).slice(0, 3);
  
  const dayAfterClasses = mockSchedule.filter(item => 
    [dayAfterName].includes(item.day)
  ).slice(0, 3);
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            {tomorrowsClasses.length > 0 ? (
              ...
              <p className="text-gray-500 py-4 text-center">No classes scheduled for tomorrow</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            {dayAfterClasses.length > 0 ? (
              ...
              <p className="text-gray-500 py-4 text-center">No classes scheduled for {dayAfterName}</p>
            )}
          </CardContent>
        </Card>
      </div>
